...
uint8_t updateExplosion(explosion_t *explosion, uint32_t deltaTime) {
	...
	// Return explosion status if animation has played
	if ((explosion->frame >> 4) > 4) {
		return 0; // Exploding = false
	}

	// TODO: Add the explosion to buffer
	//vector_t displayPosition = {(explosion->position.x >> 10), (explosion->position.y >> 10)};
	//drawASCIIImageWithBackground(explosion->image[(explosion->frame) >> 4], displayPosition, explosion->size);
	return 1; // Exploding = true
}
