...
uint16_t generateWhiteNoise(vector_t position, uint32_t range, uint32_t seed) {
    // Large number
    uint32_t noiseValue = (229 * position.x + 241 * position.y + 257 * seed);
...
uint8_t generateValueNoise(vector_t position, uint8_t scale, uint32_t seed) {
	...
	uint8_t whiteNoiseBottomLeft = generateWhiteNoise((vector_t){gridLeft, gridBottom}, 8, seed);
	uint8_t whiteNoiseBottomRight = generateWhiteNoise((vector_t){gridRight, gridBottom}, 8, seed);
	uint8_t whiteNoiseTopLeft = generateWhiteNoise((vector_t){gridLeft, gridTop}, 8, seed);
	uint8_t whiteNoiseTopRight = generateWhiteNoise((vector_t){gridRight, gridTop}, 8, seed);
