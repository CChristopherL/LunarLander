...
void addToBuffer(uint16_t *asciiImage, vector_t position, vector_t size) {
	...
	//printf("Test");
	//printf("Image character #1:\n Image PositionY: %d\n Image SizeX: %d\n BufferSIZE: %d\n", (*bufferImagePositions)[0].y, (*bufferImageSizes)[0].x, *bufferSize);
...
void pushBuffer() {
    ...
    for (uint8_t i = 0; i < bufferSize; i++) {
        // Create padded asciiImage
        vector_t asciiImageSize = {bufferImageSizes[i].x + (padding << 1), bufferImageSizes[i].y + (padding << 1)};
        vector_t asciiImagePosition = {bufferImagePositions[i].x - padding, bufferImagePositions[i].y - padding};
        uint16_t *asciiImage = malloc(asciiImageSize.x * asciiImageSize.y * sizeof(uint16_t));
        memset(asciiImage, 0x0020, asciiImageSize.x * asciiImageSize.y * sizeof(uint16_t));

        // Copy current image to padded asciiImage
        for (uint8_t r = 0; r < bufferImageSizes[i].y; r++) {
            for (uint8_t c = 0; c < bufferImageSizes[i].x; c++) {
                asciiImage[(r + padding) * asciiImageSize.x + c + padding] = bufferImages[i][r * bufferImageSizes[i].x + c];
            }
        ...
        for (uint8_t j = 0; j < bufferSize; j++) {
            ...
            int16_t overlapStartX = clamp(bufferImagePositions[j].x, asciiImagePosition.x, asciiImagePosition.x + asciiImageSize.x);
            int16_t overlapEndX = clamp(bufferImagePositions[j].x + bufferImageSizes[j].x, asciiImagePosition.x, asciiImagePosition.x + asciiImageSize.x);
            int16_t overlapStartY = clamp(bufferImagePositions[j].y, asciiImagePosition.y, asciiImagePosition.y + asciiImageSize.y);
            int16_t overlapEndY = clamp(bufferImagePositions[j].y + bufferImageSizes[j].y, asciiImagePosition.y, asciiImagePosition.y + asciiImageSize.y);

            // Translate to indices within asciiImage and bufferImage
            int16_t asciiStartX = overlapStartX - asciiImagePosition.x;
            int16_t asciiStartY = overlapStartY - asciiImagePosition.y;
            int16_t bufferStartX = overlapStartX - bufferImagePositions[j].x;
            int16_t bufferStartY = overlapStartY - bufferImagePositions[j].y;
            int16_t overlapWidth = overlapEndX - overlapStartX;
            int16_t overlapHeight = overlapEndY - overlapStartY;

            // Merge overlapping regions
            for (uint16_t r = 0; r < overlapHeight; r++) {
                for (uint16_t c = 0; c < overlapWidth; c++) {
                    uint16_t bufferPixel = bufferImages[j][(bufferStartY + r) * bufferImageSizes[j].x + (bufferStartX + c)];
                    uint16_t *asciiPixel = &asciiImage[(asciiStartY + r) * asciiImageSize.x + (asciiStartX + c)];

                    // Only replace pixel if it is not a space
                    if (j > i && (bufferPixel & 0x00FF) != 0x0020) {
                        *asciiPixel = bufferPixel;
                    } else if (((*asciiPixel) & 0x00FF) == 0x0020) {
                    	*asciiPixel = bufferPixel;
                    }
                }
            }
