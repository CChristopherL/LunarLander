...
void initLander(lander_t *lander, int16_t fuel, int8_t lives, uint32_t score, uint8_t dockingSize) {
		...
		if (lander->bulletType == 0 && lander->bulletDebounce > 64) { // Single-bullet type
			for (uint8_t i = 0; i < lander->bulletCount; i++) {
				if ((lander->bullets[i].frame >> 7) > 0) {
					...
					initBullet(&(lander->bullets[i]), (vector_t){lander->position.x + (lander->boostParticleOffsets[lander->dockingSize][lander->direction >> 4].x << 10), lander->position.y + (lander->boostParticleOffsets[lander->dockingSize][lander->direction >> 4].y << 10)}, (vector_t){bulletDirection.x * 128, bulletDirection.y * 64});
...
static void checkCollisions(lander_t *lander, uint32_t deltaTime) {
	vector_t landerCollisionPosition = {(lander->position.x >> 10) + (lander->imageSize.x >> 1), (lander->position.y >> 10) + lander->imageSize.y - 1};
...
void updateLander(lander_t *lander, uint32_t deltaTime, uint8_t joystickState) {
	...
	vector_t displayPosition = {(lander->position.x >> 10), (lander->position.y >> 10)};
