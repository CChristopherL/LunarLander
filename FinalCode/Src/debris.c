...
void initDebris(debris_t *debris, vector_t position, vector_t velocity, uint8_t type) {
	static uint16_t debrisImage[3][5][24] = {
			{
	...
	static vector_t debrisSize = {6, 4};
...
uint8_t updateDebris(debris_t *debris, uint32_t deltaTime) {
	...
	vector_t displayPosition = convertToDisplayPosition(debris->position);
	addToBuffer(debris->image[debris->type][(debris->frame) >> 4], displayPosition, debris->size);
